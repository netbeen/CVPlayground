...
int train() {
	int imgNum =2330; //2330;
	int badImgNum = 0;
	int candidatePixelNum = 400;
	int fernPixelNum = 5;
	int firstLevelNum = 10; //10
	int secondLevelNum = 120; //500
	int landmarkNum = 114;//194
	...
	cascadeFrontalface.load("/home/netbeen/koestinger_cascade_aflw_lbp.xml");
	...
	for (int i = 0; i < imgNum; i++) {
		...
		if (detectFace(tempImg, cascadeFrontalface, 5, tempBoundingBox)) {
			...
			boundingBox.push_back(tempBoundingBox);
			...
			circle(tempColorImg, Point2d(tempLandmarkMat(67, 0), tempLandmarkMat(67, 1)), 3, Scalar(255, 0, 0), -1, 8, 0);
	...
	regressor.train(images, groundTruthShapes, boundingBox, firstLevelNum, secondLevelNum, candidatePixelNum, fernPixelNum, initialNumber);
